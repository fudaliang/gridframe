  ...
  <el-row class="warp">
    ...
    <el-col :span="24" class="warp-main" v-loading="loading" element-loading-text="拼命加载中">
      <tree-table :data="menudata" :columns="columns" border highlight-current-row v-loading="loading"
        ...
        <el-table-column label="模块类型" prop="object.component"></el-table-column>
        <el-table-column label="权限" prop="object.perms">
          <template slot-scope="scope">
            <el-tag v-if="scope.row.object.perms === null"></el-tag>
            <el-tag v-if="scope.row.object.perms === 0 && scope.row.object.type != 2 ">非命令</el-tag>
            <el-tag v-if="scope.row.object.perms === 0 && scope.row.object.type == 2 ">禁用命令</el-tag>
            <el-tag v-if="scope.row.object.perms === 1">公共命令</el-tag>
            <el-tag v-if="scope.row.object.perms === 2">空闲</el-tag>
            <el-tag v-if="scope.row.object.perms === 4">其它命令</el-tag>
            <el-tag v-if="scope.row.object.perms === 8">管理命令</el-tag>
            <el-tag v-if="scope.row.object.perms === 16">删除命令</el-tag>
            <el-tag v-if="scope.row.object.perms === 32">更新命令</el-tag>
            <el-tag v-if="scope.row.object.perms === 64">创建命令</el-tag>
            <el-tag v-if="scope.row.object.perms === 128">查询命令</el-tag>
          </template>
      ...
      <el-dialog title="新增" :visible.sync="addFormVisible" :close-on-click-modal="false">
        <el-form :model="addForm" label-width="80px" :rules="editFormRules" ref="addForm">
          ...
          <el-form-item label="权限" prop="perms">
            ...
            <el-radio-group v-model="addForm.perms">
              <el-radio :label="0">禁用指令</el-radio>
              <el-radio :label="64">创建命令</el-radio>
              <el-radio :label="16">删除命令</el-radio>
              <el-radio :label="32">更新命令</el-radio>
              <el-radio :label="128">查询命令</el-radio>
              <el-radio :label="8">管理命令</el-radio>
              <el-radio :label="1">公共命令</el-radio>
              <el-radio :label="4">其它命令</el-radio>
            </el-radio-group>
          </el-form-item>
      ...
      <el-dialog title="编辑" :visible.sync="editFormVisible" :close-on-click-modal="false">
        <el-form :model="editForm" label-width="80px" :rules="editFormRules" ref="editForm">
          ...
          <el-form-item label="路径" prop="url">
            <el-radio-group v-model="editForm.perms">
              <el-radio :label="0">禁用指令</el-radio>
              <el-radio :label="64">创建命令</el-radio>
              <el-radio :label="16">删除命令</el-radio>
              <el-radio :label="32">更新命令</el-radio>
...
<script>
  ...
  import API_PARA from '../../../api/api_syspara';
  export default {
    name: 'treeTableDemo',
    ...
    methods: {
      ...
      //在菜单类型输入框输入完毕时，检查输入内容是否合法。
      componentLostFocus(item){

        let searchStr= item.target.value;
        let that = this;
        let str=  searchStr  ? that.sysParaType.filter(d => d.value.indexOf(searchStr) > -1) : that.sysParaType;
        if (str[0].id.length <= 1){
          alert("请现在系统参数中增加func_component类型参数！")
        }
      },
      //处理用户对输入框建议的选择
      handleUserSelect(item) {
        ...
        that.addForm.component ='';
        that.editForm.component  = '';

        if (that.addFormVisible ){
          that.addForm.component = item.id;
        }

        if (that.editFormVisible ){
          that.editForm.component  = item.id;
        }
      ...
      search: function () {
        ...
        //获取所有参数类型
        let paraQueryStr = {
          paraType : 'func_component'
        }
        API_PARA.listByParaType(paraQueryStr).then(function (result) {
          if (result) {
            that.sysParaType = result.map(u => ({value: u.paraName ,id:u.value}));
          }
        }, function (err) {
          that.loading = false;
